...
1. **Templates are versioned and immutable once used.** Every record stores
   ...
   continue to render against the version they were signed under. In the bundle this
   is a second JSON file at the next `rev` (`code@rev` is the version id), both
   registered in `web/src/templates.ts`; `currentRevisions` picks the highest rev per
   code for the New-record dialog, and everything that resolves an existing record
   searches the full list. First instance: `inspection-request-form.json` (IRF@A,
   frozen) and `inspection-request-form-rev-b.json` (IRF@B, current), 2026-09-13.
...
| Answer column on the Chilled Water System Flushing Record | **Add one, as an addition to the paper.** The source sheet (`spec/Chemical_Flush_Checklist.html`, template `chemical-flush-checklist.json`, code CWF) gives every item a Comments cell and no tick column, so a converted record would answer nothing and contribute no outstanding items (§6). The template therefore renders a Yes / No / NA `status` control ahead of the sheet's Comments cell, on every section and on added rows, with outcomes pass / fail / na. Decided with the user 2026-08-10. The print standard (§7, "matching the original wins") still holds in the direction that matters: the output is the source **plus** one column — every worded item is asked verbatim, in the sheet's order, and the Comments cell is kept — never the source minus anything. Pinned by `chemical-flush-checklist.source-parity.test.ts` so a later "restore parity" edit cannot drop either half. One further addition, forced by the renderer rather than chosen: the items are **numbered 1–14** where the paper numbers nothing, because `StandardTable` always prints a serial column and an empty one rules a blank column down a signed document. Print-parity checked 2026-08-10 against the source PDF — two sheets, the same count the source HTML itself prints |
| Engineer-added table columns | **A flat `dynamic_table` may let the engineer add and delete COLUMNS, on sections flagged `add_columns`** — the second scoped exception to Hard Rule #5, alongside `allow_add_rows`. Forced by the air-measurement sheets (`fcu-supply-fresh-air-measurement.json` FAM, `ahu-supply-fresh-air-measurement.json` AAM), whose source HTML carries its own `+ col` / `×` controls because the number of test points across a duct is decided at the duct, not by the template; the earlier fixed-column compromise made a different traverse a template revision, which is wrong for a quantity that is a property of the duct. Requested by the user 2026-08-11. **Headings are positional, ids are stable**: the block declares `id_prefix` / `label_prefix` / type / unit / width / align / `min_count`, `columns` seeds the starting set, and a column's printed label is always `"${label_prefix} ${position}"` — so deleting the third of seven renumbers the rest exactly as the source does, while each reading stays under the column id it was typed into and can never migrate to a different test point. Deleting a column removes its readings; deletion refuses below `min_count` (default 1); a new column never reuses a deleted id, which would inherit stale cells. The record's column list lives in `RecordValues.columns` (ids per section) — **optional on the type**, so every record written before this existed still renders off the template; read it through `columnsFor`, never by direct index, exactly as `groups` is read through `groupsFor`. Screen and print both render the effective columns, so a test point added on site prints on the signed document. **No wording is ever authored by the engineer** — the heading is generated from the position — so unlike an added row this exception carries no record-authored text at all, which is why it sits comfortably inside Rule #5's intent |
| Inspection Request (RFI) cover page | **Opt-in, one shared component.** A "Include Inspection Request cover page" toggle at the print step prints an A4-portrait cover as page 1 in front of the record (`PrintRfiCover`, driven by `buildRfiCoverData` — never per-template markup, Hard Rule #4). App-filled fields (project, floor/area, date, activity, ref, contractor sign-off) are seeded best-effort from record/template data but editable at print; the **discipline** is user-chosen (a radio + Other free-text), not auto-bound to the template — a template may pre-select a default but the user's choice prints. IRF No., Scope/Remarks, and the two blank sign-off blocks — INSPECTOR / ENGINEER ("Witnessed By") and AUTHORITY ("Approved By"), after the contractor's ("Conducted By"); three blocks since the 2026-09-13 paper revision — print as blank boxes for on-site handwriting; the **Inspection Result** (PASS / FAIL / CONDITIONAL PASS) is user-chosen at the print step, blank by default so it can still be handwritten (settled with the user 2026-09-12). The toggle and the edited options **persist on the record** as `values.rfi_cover` (`coverStateOf` / `withCoverState`, `lib/rfi-cover.ts`) and go through the ordinary autosave, so a cover ticked before "Save record" is still ticked when the form is reopened on any device (settled 2026-09-12); a locked record keeps its cover choice on screen only, since its values can no longer be written. Layout replicates `spec/reference/inspection-request-form.html`. **Mixed orientation** (option a): the cover keeps A4 portrait via a named `@page rfi-cover` rule even when the record prints landscape (e.g. heat-load), so one print action produces the correct mixed job; record pages keep PrintView's global `@page`. Verify per repo standard by printing the reference HTML and the app cover to PDF and comparing |
| Inspection Request as a record of its own (IRF) | **Both, and they do not overlap.** The print-step cover above stays exactly as specified — it is the fast path: one record, one signer, the inspector writes on the paper. Alongside it, the same sheet is also a TEMPLATE (`spec/templates/inspection-request-form.json`, code IRF), so a request can be a record with its own serial, status workflow and captured signatures (two in Rev A, three in Rev B). Use the template when the cover cannot serve: (a) **one request covering several test records** — an `attached_records` `dynamic_table` lists each covered form by title and Record No.; this table is the one block NOT on the paper, and is the reason the template exists; (b) **the inspector / engineer is not the person who signed the test form** — `sig_inspector` (`stage: "client"`) is an ordinary slot, so they sign on device or through a remote sign-off link, and `sig_contractor` (`stage: "contractor"`) gates draft → completed independently. The covered records are a TYPED LIST, not a foreign key: a cross-record reference is not expressible in the template schema (Hard Rule #4) and would tie a signed request to records that can still change status underneath it. Discipline is a per-record `dropdown` over the cover’s own five options, never bound to the template. **Rev B (2026-09-13)** follows the paper's revision to three sign-offs, one per gated step: `sig_contractor` *Conducted By* (`contractor`, draft → completed), `sig_inspector` *Witnessed By* (now `witness`, was `client` in Rev A), and the new `authority_sign_off` section's `sig_authority` *Approved By* (`client`, accepts). Pages re-split as declaration + contractor + result | inspector + authority (measured 185 / 236 / 154px headroom). Rev A stays bundled and frozen for the IRF@A records already filed (§2); new requests are always Rev B. Conditional Pass maps to `outcome: "fail"` so it lands on the outstanding-items list (§6). No top-level `footer` — both sign-offs are `sign_off` sections, because `PrintView` gives a footer a page of its own and the sheet is one A4 page. The declaration sentence and the discipline list are pinned to `lib/rfi-cover.ts` by `inspection-request-form.source-parity.test.ts` so cover and template cannot drift |
| Condensate Pipe Flood Test revised to Rev B (CPF) — Rev A withdrawn, not frozen | **Rev B (2026-09-20) replaces Rev A outright and Rev A is NOT bundled.** The paper changed form entirely — MOS-ACMV-006 Part A flood readings with requirements, Part B per-AHU flow test with a recovery percentage, Tested / Witnessed / Verified sign-off — and production D1 held no CPF@A record when the revision landed (checked 20 Sep 2026), so there was no filed record for a frozen Rev A to serve; git history keeps its definition. This is the counterpart to the IRF decision above: a superseded revision stays bundled ONLY when a record was filed under it (§2). Three conversion calls, all recorded in the template `_note`s: (a) the paper’s Requirement column (≥ 1.0 m, ≥ 30 min, ≥ 2 h, No drop, No) is template text, so each requirement is carried at the end of its row’s description and enforced by the row `limit`; (b) the durations and level drop the paper computes (A6 = A5 − A3, A8 = A4 − A7) are TYPED, because a standard row has no formula evaluation — only Part B’s Recovery is `calculated` (`collected / poured * 100`), and its `limit` of 90% is declarative today (the on-screen computed cell and the outstanding-items derivation do not evaluate a calculated column’s limit); (c) the acceptance-criteria prose is carried as the `title` of the closing field group, the free-text idiom `grease-separator-leak-test.json` set for its certification sentence, so the printed sheet still shows what PASS / FAIL was judged against |
...
An engineer-added **column** (`add_columns`, §12) is narrower still. It carries no
wording at all: the heading is generated from the column's position
(`"Test Point 3"`), never typed, so there is no record-authored text to diverge from
the template in the first place. What the engineer chooses is how many times a
measurement was taken — a quantity, like a row count — not what the measurement is
called. The template still owns the column's type, unit and meaning.
